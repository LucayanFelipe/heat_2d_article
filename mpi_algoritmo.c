...
#define CSV_JANELA_RAIO   150

#define RX  (ALPHA * DT / (DX * DX))
#define RY  (ALPHA * DT / (DY * DY))

static int rank, size;
static int local_nx, i_offset;

static double **grid_a, **grid_b;
static double **grid, **new_grid;
static double **global_grid = NULL;

static double tempos[N_MAX];

static double **alocar_matriz(int nrows, int ncols) {
    double **m = malloc((size_t)nrows * sizeof(double *));
    double *dados = malloc((size_t)nrows * ncols * sizeof(double));
    for (int i = 0; i < nrows; i++) m[i] = dados + (size_t)i * ncols;
    return m;
}

static void definir_decomposicao() {
    int base  = NX / size;
    int resto = NX % size;
    if (rank < resto) {
        local_nx = base + 1;
        i_offset = rank * (base + 1);
    } else {
        local_nx = base;
        i_offset = resto * (base + 1) + (rank - resto) * base;
    }
    if (local_nx <= 0) {
        if (rank == 0)
            fprintf(stderr, "[FATAL] Numero de processos excede NX=%d.\n", NX);
...
void inicializar() {
    for (int i = 0; i < local_nx + 2; i++) {
        memset(grid_a[i], 0, sizeof(double) * NY);
        memset(grid_b[i], 0, sizeof(double) * NY);
    }
    grid     = grid_a;
    new_grid = grid_b;

    int gi = NX / 2, gj = NY / 2;
    if (gi >= i_offset && gi < i_offset + local_nx)
        grid[gi - i_offset + 1][gj] = 100.0;
}

static void trocar_halo(double **g) {
    int cima  = (rank > 0)        ? rank - 1 : MPI_PROC_NULL;
    int baixo = (rank < size - 1) ? rank + 1 : MPI_PROC_NULL;

    MPI_Sendrecv(g[1], NY, MPI_DOUBLE, cima, 0,
                 g[local_nx + 1], NY, MPI_DOUBLE, baixo, 0,
                 MPI_COMM_WORLD, MPI_STATUS_IGNORE);

    MPI_Sendrecv(g[local_nx], NY, MPI_DOUBLE, baixo, 1,
                 g[0], NY, MPI_DOUBLE, cima, 1,
                 MPI_COMM_WORLD, MPI_STATUS_IGNORE);
}
...
void atualizar() {
    ...
    for (int li = 1; li <= local_nx; li++) {
        int gi = i_offset + li - 1;
        if (gi == 0 || gi == NX - 1) continue;
        for (int j = 1; j < NY - 1; j++) {
            ng[li][j] = g[li][j]
                + RX * (g[li+1][j] - 2.0*g[li][j] + g[li-1][j])
                + RY * (g[li][j+1] - 2.0*g[li][j] + g[li][j-1]);
        }
    }
    ...
    for (int li = 1; li <= local_nx; li++) {
        int gi = i_offset + li - 1;
        if (gi == 0 || gi == NX - 1) {
            for (int j = 0; j < NY; j++) ng[li][j] = 0.0;
        } else {
            ng[li][0]      = 0.0;
            ng[li][NY - 1] = 0.0;
        }
...
double energia_total() {
    ...
    for (int li = 1; li <= local_nx; li++)
        for (int j = 0; j < NY; j++)
            soma_local += grid[li][j];
    double soma_global;
    MPI_Allreduce(&soma_local, &soma_global, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    return soma_global;
}
...
double temp_maxima() {
    ...
    for (int li = 1; li <= local_nx; li++)
        for (int j = 0; j < NY; j++)
            if (grid[li][j] > mx_local) mx_local = grid[li][j];
    double mx_global;
    MPI_Allreduce(&mx_local, &mx_global, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
    return mx_global;
}

static double energia_total_global(double **g) {
    double soma = 0.0;
    for (int i = 0; i < NX; i++)
        for (int j = 0; j < NY; j++)
            soma += g[i][j];
...
static double temp_maxima_global(double **g) {
    double mx = 0.0;
    for (int i = 0; i < NX; i++)
        for (int j = 0; j < NY; j++)
...
static void coletar_grid_global() {
    int *recvcounts = NULL, *displs = NULL;
    if (rank == 0) {
        recvcounts = malloc(size * sizeof(int));
        displs     = malloc(size * sizeof(int));
    }
    int meu_count = local_nx * NY;
    MPI_Gather(&meu_count, 1, MPI_INT, recvcounts, 1, MPI_INT, 0, MPI_COMM_WORLD);
    if (rank == 0) {
        displs[0] = 0;
        for (int r = 1; r < size; r++) displs[r] = displs[r-1] + recvcounts[r-1];
    }
    MPI_Gatherv(grid[1], meu_count, MPI_DOUBLE,
                (rank == 0) ? &global_grid[0][0] : NULL,
...
void debug_visual() {
    ...
    inicializar();
    coletar_grid_global();
    if (rank == 0) { debug_snapshot(0, STEPS); exportar_csv(0); }
    ...
    for (int t = 1; t < STEPS; t++) {
        ...
        if (next < N_SNAP_LOOP && t == snap[next]) {
            coletar_grid_global();
            if (rank == 0) { debug_snapshot(t, STEPS); exportar_csv(t); }
            next++;
        }
    }

    coletar_grid_global();
    if (rank == 0) { debug_snapshot(STEPS, STEPS); exportar_csv(STEPS); }
...
void info_problema() {
    ...
    printf("  Memoria total:       %.2f MB (2 grids, distribuida)\n", 2.0*mem_mb);
...
void verificar_energia() {
    if (rank == 0) printf("===== CONSERVACAO DE ENERGIA =====\n");
    ...
    if (rank == 0) printf("  Energia em t=0:    %.6f\n", E0);
...
void aquecimento() {
    ...
    for (int w = 0; w < WARMUP_RUNS; w++) {
        ...
        double local_dt = t1 - t0, dt_max;
        MPI_Reduce(&local_dt, &dt_max, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

        if (rank == 0) {
            printf("  Warmup %d/%d: %.3f s (descartado)\n", w+1, WARMUP_RUNS, dt_max);
            fflush(stdout);
        }
    }
    if (rank == 0) printf("\n");
...
void benchmark() {
    if (rank == 0) {
        printf("===== BENCHMARK - Perfil %s (MPI, %d processos) =====\n", PERFIL_NOME, size);
    ...
    while (!parar) {
        ...
        MPI_Bcast(&parar, 1, MPI_INT, 0, MPI_COMM_WORLD);
    }
...
int main(int argc, char **argv) {
    ...
    definir_decomposicao();

    grid_a = alocar_matriz(local_nx + 2, NY);
    grid_b = alocar_matriz(local_nx + 2, NY);
    grid     = grid_a;
    new_grid = grid_b;

    if (rank == 0) {
        global_grid = alocar_matriz(NX, NY);

        printf("==============================================\n");
        printf("  heat2d_mpi - Difusao de Calor 2D\n");
        printf("  Versao: MPI (%d processos)  Perfil: %s\n", size, PERFIL_NOME);
