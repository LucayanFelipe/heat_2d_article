...
void exportar_csv(int passo) {
    ...
    fprintf(f, "# heat2d_omp - Perfil: %s\n", PERFIL_NOME);
    fprintf(f, "# passo=%d  t_fisico=%.6f  DT=%.6f\n", passo, passo * DT, DT);
    fprintf(f, "# NX=%d  NY=%d  ALPHA=%.6f  DX=%.6f  DY=%.6f\n", NX, NY, ALPHA, DX, DY);
    fprintf(f, "# RX=%.6f  RY=%.6f  STEPS=%d  threads=%d\n", RX, RY, STEPS, omp_get_max_threads());
...
void benchmark() {
    printf("===== BENCHMARK - Perfil %s (OpenMP, %d threads) =====\n", PERFIL_NOME, omp_get_max_threads());
...
int main() {
    ...
    printf("==============================================\n");
    printf("  heat2d_omp - Difusao de Calor 2D\n");
    printf("  Versao: OpenMP (%d threads)  Perfil: %s\n", omp_get_max_threads(), PERFIL_NOME);
